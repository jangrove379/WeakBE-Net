...
def calculate_agreement():
    ...
    df = pd.read_csv(f"{args.experiment_dir}/predictions.csv")

    summary = bootstrap_all_metrics(df)

    results.append(
        {
            "experiment": args.experiment_dir.replace(".csv", "").replace("experiments/", ""),
...
def plot_multiclass_ece_separate(bin_width=0.1, label_col="cons_label", conf_base="softmax"):
    ...
    if "/dn" in args.experiment_dir:
        label = "DN"
        color = colors["dn"]
    elif "/wdn" in args.experiment_dir:
        label = "WDN"
        color = colors["wdn"]
